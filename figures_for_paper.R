...
fig2c <- fig2d <- fig2e <- ggplot() + geom_sf(data = clusters,
      ...
      ggtitle("Required pumps per sq. km")+
     xlab("")+
     ylab("")
...
fig2d <- ggplot() + geom_sf(data = clusters,
  ...
  theme(legend.position = c(0.2, 0.28), legend.direction = "vertical", legend.text=element_text(size=8), axis.line=element_blank(),axis.text.x=element_blank(), legend.title=element_text(size=8, face = "bold"), panel.border = element_rect(colour = "black", fill=NA, size=1),
        ...
        axis.title.x=element_blank(),
        axis.title.y=element_blank(), plot.title = element_text(size = 10, hjust = 0.5))+
  ggtitle("PV installation requirements (kW per sq. km)")+
  xlab("")+
  ylab("")
...
clusters$total_revenues_discounted_discounted_yearly_cl <- ifelse(is.na(clusters$profit_yearly) , 0, clusters$total_revenues_discounted_discounted_yearly)
...
colnames(data) <- c("region", "Pump costs", "PV costs", "Transport costs", "Potential revenues", "Potential profits")
...
data$name <- reorder(data$name, new.order=c(2,1,3,4,5))
...
fig3 <- ggplot(data)+
  ...
  xlab("Region")+
  ggtitle("")+
  scale_fill_manual(name="", values=ggsci::pal_npg("nrc", alpha = .8)(9)[c(7, 3, 1, 2, 6)])+
  scale_x_discrete(labels=c("SSA-wide", "East Africa", "Central Africa", "Southern Africa", "West Africa"))
  

#

clusters_si <- read_rds(paste0("clusters_with_data_7_", paste(scenarios[1,], collapse="_"), ".Rds"))

clusters_si <- clusters_si

clusters_si <- filter(clusters_si, profit_yearly>0 & !is.na(profit_yearly))

fig3b <- ggplot(arrange(clusters_si, -profit_yearly/A_total))+theme_classic()+
  geom_line(aes(x=cumsum(A_total), y=profit_yearly/A_total, colour=which_pumping))+
  scale_x_log10()+
  ggsci::scale_colour_npg(name="Optimal irrigation")+
  xlab("(Potentially) irrigated cropland area (million ha, log scale)")+
  ylab("Potential yearly profits (USD/ha)")
